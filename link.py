...
class Link:
    ...
    def __init__(self, prompt_template:str, constraints: List[constraint.Constraint], executor, *args, **kwargs):
    ...
    def execute(self, conversation:conversation.Conversation, repeat_threshold:int=5, best_effort:bool=False)->conversation.Conversation:
        ...
        for i in range(repeat_threshold):
            ...
            if feedback == "":
                conversation.add_message({"role":"assistant", "content":result, "keep":True})
                conversation.clean()
                break
            else:
                ...
                conversation.add_message({"role":"user", "content":feedback, "keep":False})
        return conversation
